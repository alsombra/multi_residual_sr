...
#                 data_iter = iter(self.data_loader)  
            ...
            data_dict = imgs
            img_paths, lr_image, hr_image, x, y = data_dict['image_path'], \
                                                  data_dict['lr_image_scaled'], \
                                                  data_dict['hr_image_scaled'], \
                                                  data_dict['lr_image_with_resid'], \
                                                  data_dict['hr_residual_scaled']
            ...
            if (i+1) % self.log_step == 0:
                print("[{}/{}] epoch: {} loss: {:.5f}".format(i+1,  len(self.data_loader), step, loss.item()))
            ...
            if (i+1) % self.sample_step == 0:
                ...
                imgs_comb = self.img_add_info(img_paths, tmp, i+1, loss)                
                #from IPython.display import display
                grid_PIL = imgs_comb
                grid_PIL.save('./samples/epoch_{}_train_{}.jpg'.format(step, i + 1))
                if self.data_loader.batch_size == 1: #only saves separate images if batch == 1
                    ...
                    lr_image_np.save('./samples/epoch_{}_train_{}_lr.png'.format(step, i+1))
                    hr_bic.save('./samples/epoch_{}_train_{}_hr_bic.png'.format(step, i+1))
                    hr_hat.save('./samples/epoch_{}_train_{}_hr_hat.png'.format(step, i+1))
                    hr.save('./samples/epoch_{}_train_{}_hr.png'.format(step, i+1))

            # Save check points
            if (i+1) % self.model_save_step == 0:                
                self.save(step+1, loss.item(), os.path.join(self.model_save_path, '{}.pth.tar'.format(self.loss_function.upper() + '_' str(step) + '_' + str(i+1))))
